...
## 출력 색상 구분
*   **초록색**: 메인 에이전트의 응답 (작업 계획 및 설명)
*   **청록색**: 서브 에이전트의 응답 (실제 작업 수행)
*   **노란색**: 파일 미리보기 및 터미널 출력
*   **빨간색**: 오류 메시지
...
*   메인 에이전트는 사용자와 직접 대화하며, 서브 에이전트의 존재를 숨깁니다.
*   작업은 순차적으로 수행되며, 병렬 처리는 지원하지 않습니다.
*   자동 승인 모드(`/allow`)를 사용하면 수동 승인 없이 작업을 자동으로 수행합니다.
